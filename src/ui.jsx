/* =========================================================
   UI primitives — icons, toast, modal, charts
   ========================================================= */

const { useState, useEffect, useMemo, useRef, useCallback, createContext, useContext } = React;

/* ---------- Icons (stroke-only, original) ---------- */
...
/* ---------- Sparkline / Bar / Donut ---------- */
function Sparkline({data, color="var(--tea)", height=60, fill=true}){
...
function Bars({data, color="var(--amber)", height=140, labels}){
...
/* ---------- Sidebar layout ---------- */
function AppShell({user, onLogout, nav, current, onNav, children}){
  return (
        ...
        <div style={{borderTop:"1px solid rgba(246,241,231,.12)", paddingTop:14}}>
          <div style={{display:"flex", alignItems:"center", gap:10, padding:"6px 8px"}}>
            ...
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:13, fontWeight:500, color:"#fffdf7"}}>{user.label}</div>
...
function Logo({size=48, dark=false}){
  // Original geometric mark: tea cup with a leaf and rising "steam" dots
...
/* ---------- StatCard ---------- */
function StatCard({label, value, sub, delta, accent="var(--amber)", icon, trend}){
  return (
    <div className="card hover" style={{padding:"20px 22px", display:"flex", flexDirection:"column", gap:8, position:"relative", overflow:"hidden"}}>
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between"}}>
        ...
        {icon && (
          <div style={{
            ...
            background: `color-mix(in oklch, ${accent} 15%, white)`,
            color: accent,
            display:"flex", alignItems:"center", justifyContent:"center"
          }}>
      ...
      <div className="num font-display" style={{fontSize:38, fontWeight:600, lineHeight:1, letterSpacing:"-0.02em", color:"var(--ink)"}}>
        {value}
      </div>
      {(sub || delta) && (
        <div style={{display:"flex", alignItems:"center", gap:10, fontSize:13, color:"var(--ink-3)"}}>
          {delta && (
            <span style={{
              ...
              color: delta.startsWith("-") ? "var(--bad)" : "var(--ok)",
              fontWeight:500
            }}>
              <Icon name={delta.startsWith("-")?"arrow-down":"arrow-up"} size={12}/>
              {delta}
            </span>
          )}
          {sub && <span>{sub}</span>}
...
/* ---------- Empty state ---------- */
function Empty({icon="leaf", title, subtitle}){
  return (
...
Object.assign(window, {
  Icon, ToastProvider, useToast, Modal,
  Sparkline, Bars, Donut, AppShell, PageHeader,
  Logo, StatCard, SearchBox, Empty, Seg,
});
