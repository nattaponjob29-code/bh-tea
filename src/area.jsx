/* =========================================================
   AREA MANAGER — multi-branch dashboard
   ========================================================= */

function AreaView({user, store, refresh}){
  const [page, setPage] = useState("dashboard");

  // My branches = branches in any of my responsible areas
  ...
  return (
    <AppShell user={user} onLogout={()=>window.__doLogout()} nav={nav} current={page} onNav={setPage}>
      ...
      {page==="branches"      && <AreaBranches  user={user} myBranches={myBranches} records={myRecords} store={store}/>}
      {page==="history"       && <SplitHistoryPage records={myRecords} store={store}
                                    title="ประวัติการผลิต" eyebrow="History"
                                    showBranch={true} showBranchFilter={true} refresh={refresh}/>}
      {page==="defectHistory" && <DefectHistoryPage records={myRecords} store={store}
                                    title="ประวัติเสียหาย" eyebrow="Defect History"
                                    showBranch={true} showBranchFilter={true} refresh={refresh}/>}
    </AppShell>
  );
}
  ...
  return (
    <>
      ...
      <div className="card" style={{padding:"24px 26px", marginBottom:22}}>
        ...
        <StackedBars labels={days.map(d => new Date(d).toLocaleDateString("th-TH",{day:"2-digit",month:"short"}))}
                     series={[
            ...
            {byBranch.map((b,i)=>{
              ...
              return (
                <div key={b.branch.id} style={{display:"flex", flexDirection:"column", gap:6}}>
                  ...
                  <div className="bar"><i style={{width: `${(b.prod/max)*100}%`}}/></div>
                </div>
              );
            })}
          </div>
...
function Legend({color, label}){
  return <span style={{display:"inline-flex", alignItems:"center", gap:6, color:"var(--ink-3)"}}>
      ...
      {labels.map((lab,i)=>{
        ...
        return (
          <div key={i} style={{flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:6, height:"100%"}}>
            <div style={{flex:1, display:"flex", flexDirection:"column-reverse", width:"100%", maxWidth:48, alignSelf:"center", justifySelf:"flex-end"}}>
              {series.map((sr,j)=>{
                const v = sr.data[i]||0;
                const h = (v/max)*100;
                return v ? (
...
/* ---------- Area > Branches detail ---------- */
function AreaBranches({user, myBranches, records, store}){
  const initEnd = todayISO();
  const initStart = (() => { const d = new Date(); d.setDate(d.getDate()-6); return d.toISOString().slice(0,10); })();
  const [from, setFrom] = useState(initStart);
  const [to, setTo]     = useState(initEnd);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="คลิกการ์ดเพื่อดูรายการในสาขานั้น"
        right={<DateRangePicker from={from} to={to} setFrom={setFrom} setTo={setTo}/>}
      />
...
Object.assign(window, { AreaView, DateRangePicker, StackedBars, Legend, DefectMaterialDashboard });

/* ---------- Shared: Defect by material dashboard widget ----------
   Used by Area & Admin dashboards. Aggregates defect records into
   material quantities using bomDefect × grams.
*/
function DefectMaterialDashboard({records, store}){
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:30}}>
            ...
            {top.map((b,i) => (
              <div key={b.code} style={{display:"flex", flexDirection:"column", gap:6}}>
                <div style={{display:"flex", justifyContent:"space-between", fontSize:13, gap:10}}>
                  ...
                  <span className="num" style={{color:"var(--bad)", fontWeight:600, whiteSpace:"nowrap", fontFamily:"Space Grotesk"}}>
          ...
          <div>
            ...
            <div style={{marginTop:16, padding:"12px 14px", background:"var(--bg)", borderRadius:10, border:"1px solid var(--line)", fontSize:12, color:"var(--ink-3)"}}>
              ...
              <br/>
              ตัวอย่าง: ลงเสีย 1000g + BOM 0.001 ถุง/g = 1 ถุง
            </div>
          </div>
        </div>
      )}
